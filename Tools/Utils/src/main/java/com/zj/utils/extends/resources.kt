...
fun androidx.fragment.app.Fragment.loadColor(@ColorRes id: Int): Int = activity?.loadColor(id) ?: 0

fun androidx.fragment.app.Fragment.loadDrawable(@DrawableRes id: Int): Drawable? = activity?.loadDrawable(id)

fun androidx.fragment.app.Fragment.loadRaw(@RawRes id: Int): InputStream? = activity?.loadRaw(id)

fun androidx.fragment.app.Fragment.loadRaw(@RawRes id: Int, value: TypedValue): InputStream? = activity?.loadRaw(id, value)

fun androidx.fragment.app.Fragment.loadAsset(fileName: String, accessMode: Int = AssetManager.ACCESS_STREAMING): InputStream? = activity?.loadAsset(fileName, accessMode)

fun androidx.fragment.app.Fragment.loadTypefaceFromAsset(fileName: String): Typeface? = activity?.loadTypefaceFromAsset(fileName)
