...
fun androidx.fragment.app.Fragment.getUriFromFile(file: File, authority: String = "${activity?.packageName}.fileprovider"): Uri {
    return activity?.getUriFromFile(file, authority) ?: Uri.EMPTY
}

fun androidx.fragment.app.Fragment.setIntentDataAndType(intent: Intent, type: String, file: File, authority: String, writeEnable: Boolean) {
    activity?.setIntentDataAndType(intent, type, file, authority, writeEnable)
}