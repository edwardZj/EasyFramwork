...
fun androidx.fragment.app.Fragment.getVersionName(pkgName: String = activity?.packageName ?: ""): String {
    return activity?.getVersionName(pkgName) ?: ""
}

fun androidx.fragment.app.Fragment.getVersionCode(pkgName: String = activity?.packageName?:""): Int {
    return activity?.getVersionCode(pkgName) ?: -1
}

fun androidx.fragment.app.Fragment.installApp(file: File, authority: String, writeEnable: Boolean) {
    activity?.installApp(file, authority, writeEnable)
}

fun androidx.fragment.app.Fragment.isInstallApp(pkgName: String): Boolean {
    return activity?.isInstallApp(pkgName) ?: false
}

fun androidx.fragment.app.Fragment.isAppForeground(pkgName: String = activity?.packageName ?:""): Boolean {
    return activity?.isAppForeground(pkgName) ?: false
}
